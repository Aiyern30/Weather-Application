  ...
  useEffect(() => {
    const fetchWeatherData = async (query: string) => {
      try {
        ...
        // Fetch forecast weather based on the selected number of days
    ...
    try {
      const response = await axios.get(
        `https://api.unsplash.com/search/photos`,
        {
          params: {
            query: query,
            client_id: process.env.NEXT_PUBLIC_UNSPLASH_ACCESS_KEY, // Add your Unsplash API key here
            per_page: 1,
          },
        }
      );
  ...
  useEffect(() => {
    const fetchSuggestions = async (query: string) => {
      ...
      try {
        ...
        const data = await response.json();
        setSuggestions(data.map((item: any) => item.name)); // Update with location names
      } catch (error) {
